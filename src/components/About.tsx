...
<p className='mb-10 max-w-2xl font-Ovo'>I am a skilled Full-Stack Developer specializing in the MERN stack, with expertise in React, Next.js, Vue, and jQuery for frontend development. On the backend, I work with Node.js and Express, utilizing MongoDB, PostgreSQL, and SQL databases. Passionate about building scalable solutions, I excel in software development and IT project management.</p>
          ...
          <motion.ul initial={{opacity: 0 }} whileInView={{opacity: 1 }} transition={{duration: 0.8, delay: 1}} className='grid grid-cols-1 sm:grid-cols-3 gap-6 max-w-2xl'>
            {infoList.map(({icon, iconDark, title, description}, index) => (
                <motion.li whileHover={{scale: 1.05}}  key={index} className='border-[0.5px] border-gray-400 rounded-xl p-6 cursor-pointer hover:bg-lightHover hover:-translate-y-1 duration-500 hover:shadow-black dark:border-white dark:hover:shadow-white dark:hover:bg-darkHover/50'>
                    ...
                    <h3 className='my-43 font-semibold text-gray-700 dark:text-white'>{title}</h3>
...
<motion.ul initial={{opacity: 0 }} whileInView={{opacity: 1 }} transition={{duration: 0.6, delay: 1.5}}  className='flex items-center gap-3 sm:gap-5'>
    {toolsData.map((tool, index)=> (
        <motion.li whileHover={{ scale: 1.1}} className= "flex items-center justify-center hover:-translate-y-1 duration-500  w-12 sm:w-14 aspect-square border border-gray-400 rounded-lg cursor-pointer " key={index}><Image src={tool} alt='Tool' className='w-5 sm:w-7'/></motion.li>
    ))}
</motion.ul>
            </motion.div>
        </motion.div>
